...
- 仓鼠增益：`超燃咚咚 / CCId 1225` 与 `咚咚肾上腺素 / CCId 1186` 都使用 180 秒固定时长，但结束提醒只认实际 `EventId 5`。肾上腺素默认在同一时刻已获得 `净化之浪 / CCId 645` 时静默结束提醒，对应鲸鱼覆盖场景；设置页可关闭此静默。
- `stack_alert`：叠层达到指定层数时提醒。湛蓝内伤使用该规则，并且只有在危险提醒已经触发后、状态自然清除时，才按设置播放消除提示。
- `cooldown_alert`：按各技能已确认的可靠起点推算冷却完成。魔法阵使用本人 `SkillId 10103 / EventId 12`（服务器接受施放）后固定 140 秒；第三只眼使用效果首次开始后 240/300 秒；半神化仍可按状态结束后延迟计时。
...
| 超燃咚咚 | 1225 | 剩余提醒子开关默认关闭；时间预填 30 秒 | 结束提醒子开关默认关闭；实际移除事件触发 |
| 咚咚肾上腺素 | 1186 | 剩余提醒子开关默认关闭；时间预填 30 秒 | 结束提醒子开关默认关闭；鲸鱼覆盖静默默认开启 |
| 活力之歌 | 800 | 30 秒 | 是 |
| 状态支援 | 515 | 30 秒 | 是 |
...
| 项目 | CCId | 结束提醒 | 冷却完成提醒 |
| --- | ---: | --- | --- |
| 魔法阵 | 10133 | 默认关闭 | 本人 `SkillId 10103 / EventId 12` 后固定 140 秒，默认开启且不可自定义时长 |
...
游戏暂时没有新事件时，独立启动器会保持 WebSocket 和自带后端运行，不会因为空闲而主动重连、重启或清除已经学习的 self id。只有 Mabicat 进程真实退出或 WebSocket 明确报错时才进入恢复流程；后端恢复时保留当前 self id。玩家主动退出角色并切换到其他角色时，应重启提醒器并重新开关魔法盾。
